...
class SerialDataHandler(DataHandler):
    ...
    def load_data(self):
        with open(self.meta_path) as data_file:
            ...
            # ovo spajanje za path se treba uraditi preko neke path biblioteke da bude ok i kad je korisnik na Linuksu
            self.linked_file_path = self.path.split("storage/")[0] + "storage/" + self.meta_data["linked file"]

        # ako ucitavamo podatke glavne tabele
        if self.is_parent_table == True:
            with open(self.path) as data_file:
                self.data = json.load(data_file)
                
        # ako ucitavamo podatke tabele glavne tabele
    ...
    def save(self):
        if self.is_parent_table == True:
            ...
            for d in self.loaded_data:
                data.append(d)
            i = 0
            # sklanjanje starih objekata koji su bili prikazani
            for d in data:
